...
import { InfoCard, useApi, Content, WarningPanel, Progress }
       from '@backstage/core';
...
export const SnykOverviewComponent: FC<{ entity: Entity }> = ({ entity }:{
  entity: Entity;
}) => {
              ...
              alignItems="center">
                <Grid item>
                      <WarningPanel
                        ...
                        message={
                          <>
                            Seems we are missing some references, check out the example annotations in <a href="https://github.com/snyk-tech-services/backstage-test/blob/master/java-goof-component.yaml">java-goof-component.yaml</a>.
                          </>
                        }
                      />
                </Grid>
  ...
  const { value, loading, error } = useAsync(
    async () => {
      ...
      let aggregatedIssuesCount: issuesCount = {high: 0, medium: 0, low: 0}
      ...
      const projectList = fullProjectList.projects?.filter(project => 
        project.id && utils.extractProjectIdFromAnnotations(projectIdsFromAnnotations).includes(project.id))

      let projectsCount = 0
      
      const projectIds = utils.extractProjectIdFromAnnotations(projectIdsFromAnnotations)
      for(let i=0; i<projectIds.length; i++){
        if(projectList?.some(selectedProject => selectedProject.id == projectIds[i])){
          projectsCount++
          const vulnsIssues: IssuesArray = await snykApi.ListAllAggregatedIssues(orgName, projectIds[i])
  ...
  let issuesCount: issuesCount = value?.aggregatedIssuesCount || {high: 0, medium: 0, low: 0}
  if (loading) {
    return <Content><InfoCard style={counterContentStyle} title="Issues"
    deepLink={{ title: 'Retrieving Vulnerabilities from Snyk', link: '' }} >
    <SnykCircularCounter issuesCount={issuesCount}  />
    <Progress />
  </InfoCard></Content>
  } else if (error) {
    return <Content><InfoCard style={counterContentStyle} title="Vulnerabilities"
...
const linkInfo = {title: `Across ${projectsCount} project${projectsCount > 0? 's':''}`, link: `snyk` }

  return <InfoCard style={counterContentStyle} title="Vulnerabilities" deepLink={linkInfo}>            
           <SnykCircularCounter issuesCount={issuesCount}  />
         </InfoCard>
};
