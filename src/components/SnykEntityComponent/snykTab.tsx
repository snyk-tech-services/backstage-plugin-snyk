...
import { StructuredMetadataTable, Progress, InfoCard, Content, TabbedCard, CardTab }
       from '@backstage/core';
...
import { IssuesArray, ProjectGetResponseType, DepgraphGetResponseType } from '../../types/types'
...
export const generateSnykTabForProject = (snykApi: SnykApi, orgName:string, projectId:string) => {

    return ({}) => {
        const { value, loading, error } = useAsync(
          async () => {
            const vulnsIssues: IssuesArray = await snykApi.ListAllAggregatedIssues(orgName, projectId)
            const licenseIssues: IssuesArray = await snykApi.ListAllAggregatedLicenseIssues(orgName, projectId)
            const ignoredIssues: IssuesArray = await snykApi.ListAllIgnoredIssues(orgName, projectId)
            const depGraph: DepgraphGetResponseType = await snykApi.GetDependencyGraph(orgName, projectId)
            const projectDetails: ProjectGetResponseType = await snykApi.ProjectDetails(orgName, projectId)
            return {vulnsIssues,licenseIssues,ignoredIssues,depGraph,projectDetails}
          }
        );
        ...
        const linkInfo = { title: 'More details', link: `https://app.snyk.io/org/${orgName}/project/${projectId}` };
